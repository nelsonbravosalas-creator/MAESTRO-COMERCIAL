...
export const createProjectsRouter = (pool: Pool) => {
  ...
  router.post('/', validate({ body: projectCreateSchema }), async (req: AuthRequest, res) => {
    const db = await pool.connect()
    try {
      const { quotation_id, client_id, name, status, start_date, end_date, budget, assigned_to } =
        req.body
  ...
  router.put(
    '/:id',
    validate({ params: uuidParams('id'), body: projectUpdateSchema }),
  ...
  router.post(
    '/:id/costs',
    validate({ params: uuidParams('id'), body: executionCostSchema }),
    async (req: AuthRequest, res) => {
      try {
        ...
        logger.error('Create project cost error', {
          error: error.message,
  ...
  router.delete(
    ...
    validate({ params: uuidParams('projectId', 'costId') }),
    async (req: AuthRequest, res) => {
      try {
        const result = await pool.query(
        ...
        if (result.rows.length === 0)
          return res.status(404).json({ error: 'Project cost not found' })
        return res.json({ message: 'Project cost deleted successfully' })
      } catch (error: any) {
        logger.error('Delete project cost error', {
          error: error.message,
  ...
  router.put(
    '/:id/costs/:costId',
    validate({ params: uuidParams('id', 'costId'), body: executionCostSchema }),
    async (req, res) => {
  ...
  // GET /:id/assignments
  router.get('/:id/assignments', validate({ params: uuidParams('id') }), async (req, res) => {
    const { id } = req.params
    try {
      const result = await pool.query(
  ...
  router.post(
    '/:id/assignments',
    validate({ params: uuidParams('id'), body: assignmentSchema }),
    async (req, res) => {
      const { id } = req.params
  ...
  router.delete(
    ...
    validate({ params: uuidParams('id', 'userId') }),
    async (req, res) => {
  ...
  router.post(
    '/:id/tasks',
    validate({ params: uuidParams('id'), body: taskCreateSchema }),
    async (req: AuthRequest, res) => {
      try {
        const { name, description, assignee_id, category_id, start_date, end_date, sort_order } =
          req.body
        const result = await pool.query(
  ...
  router.put(
    ...
    validate({ params: uuidParams('projectId', 'taskId'), body: taskUpdateSchema }),
    async (req: AuthRequest, res) => {
      try {
        const {
          name,
  ...
  router.delete(
    '/:projectId/tasks/:taskId',
    validate({ params: uuidParams('projectId', 'taskId') }),
    async (req: AuthRequest, res) => {
      try {
        const result = await pool.query(
