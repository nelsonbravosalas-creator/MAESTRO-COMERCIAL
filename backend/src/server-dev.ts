...
const buildQuotationFull = (q: any) => ({
  ...
  contact_name: q.contact_id
    ...
    : null,
  categories: db.quotation_categories
    .filter((c: any) => c.quotation_id === q.id)
    .sort((a: any, b: any) => a.sort_order - b.sort_order),
  line_items: db.quotation_line_items
    .filter((i: any) => i.quotation_id === q.id)
    .sort((a: any, b: any) => a.sort_order - b.sort_order),
  terms: db.quotation_terms
    .filter((t: any) => t.quotation_id === q.id)
    .sort((a: any, b: any) => a.sort_order - b.sort_order),
...
app.get('/api/config', requireAuth, (_req: Request, res: Response) => {
  ...
  db.app_config.forEach((c: any) => {
    config[c.key] = c.value
  })
  res.json(config)
})

app.patch('/api/config/:key', requireAuth, requireRole('admin'), (req: AuthReq, res: Response) => {
...
app.post('/api/auth/login', async (req: Request, res: Response) => {
  try {
    ...
    const user = db.users.find(
      (u: any) => u.email.toLowerCase() === email.toLowerCase() && !u.deleted_at
    )
    if (!user || !user.is_active)
      return res
        .status(401)
        .json({ error: 'Unauthorized', message: 'Email o contraseña inválidos' })
...
app.delete('/api/catalog/:id', requireAuth, requireRole('admin'), (req: Request, res: Response) => {
  const item = db.catalog_items.find((i: any) => i.id === req.params.id)
  if (!item) return res.status(404).json({ error: 'Item no encontrado' })
  ...
  try {
    ...
    catList.forEach((c: any) => {
      db.quotation_categories.push({
        ...
        label: c.label,
        margin_pct: Number(c.margin_pct) || Number(c.margin) || 30,
        color: c.color || null,
        note: c.note || null,
  ...
  if (Array.isArray(categories)) {
    ...
    categories.forEach((c: any) => {
      db.quotation_categories.push({
        ...
        category_id: c.category_id || c.id,
        label: c.label,
        margin_pct: Number(c.margin_pct || c.margin) || 30,
        color: c.color || null,
        note: c.note || null,
        sort_order: Number(c.sort_order) || 0,
      })
...
app.get('/api/dashboard/kpis', requireAuth, (_req: Request, res: Response) => {
  ...
  const totalFacturado = activeInvoices.reduce((s: number, i: any) => s + (i.total_amount || 0), 0)

  const totalGasto = db.execution_costs.reduce(
    (s: number, ec: any) => s + (ec.quantity || 1) * (ec.unit_price || 0),
    0
  )
  ...
  res.json({
    kpis: {
      ...
      cotizaciones_abiertas: activeQuotes.filter((q: any) =>
        ['Emitida', 'Enviada'].includes(q.status)
      ).length,
